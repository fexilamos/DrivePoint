...
<div class="container mx-auto px-4 py-6 max-w-lg">
    <h1 class="text-3xl font-bold mb-6">Editar Carro</h1>
    ...
    <form action="{{ route('carros.update', $carro) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="number" name="ano" id="ano" value="{{ old('ano', $carro->ano) }}" required
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>

        <div>
            <label for="matricula" class="block text-gray-700 font-semibold mb-2">Matrícula</label>
            <input type="text" name="matricula" id="matricula" value="{{ old('matricula', $carro->matricula) }}" required
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>

        <div>
            <label for="preco_dia" class="block text-gray-700 font-semibold mb-2">Preço por Dia (€)</label>
            <input type="number" step="0.01" name="preco_dia" id="preco_dia" value="{{ old('preco_dia', $carro->preco_dia) }}" required
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>

        <div>
            <label for="imagem" class="block text-gray-700 font-semibold mb-2">Nova Imagem (opcional)</label>
            <input type="file" name="imagem" id="imagem"
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            @if ($carro->imagem)
                <img src="{{ asset('storage/' . $carro->imagem) }}" alt="Imagem atual" class="mt-2 rounded max-w-xs" />
            @endif
        </div>

        <div class="flex items-center">
            <input type="checkbox" name="disponivel" id="disponivel" {{ $carro->disponivel ? 'checked' : '' }}
        ...
        <div class="flex space-x-4">
            <button type="submit"
                class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded">Atualizar Carro</button>
