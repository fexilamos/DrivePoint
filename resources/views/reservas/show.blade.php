...
@section('content')
<div class="max-w-3xl mx-auto mt-10 bg-white rounded-lg shadow p-8 flex flex-col sm:flex-row gap-8 items-start">
    <div class="flex-2">
        <h2 class="text-2xl font-bold mb-6 text-center sm:text-left">Detalhe da Reserva</h2>
        <div class="mb-4">
            <span class="font-semibold">Carro:</span>
            {{ $reserva->bemLocavel->marca->nome ?? '-' }} {{ $reserva->bemLocavel->modelo ?? '-' }}
        </div>
        <div class="mb-4">
            <span class="font-semibold">Matrícula:</span> {{ $reserva->bemLocavel->registo_unico_publico ?? $reserva->bemLocavel->matricula ?? '-' }}
        </div>
        <div class="mb-4">
        ...
        @if(session('success'))
            <div class="bg-white hover:bg-red-100 text-red-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow mt-4">{{ session('success') }}</div>
        @endif
        @if(session('error'))
        ...
        <div class="mt-8 text-center sm:text-left">
            <a href="{{ route('profile.edit') }}" class="text-blue-600 hover:underline"><- Voltar </a>
        </div>
    </div>
    <div class="w-full sm:w-64 flex-shrink-0 flex justify-center">
        <img src="/images/{{ $reserva->bemLocavel->imagem ?? 'default.jpg' }}" alt="Imagem do carro" class="rounded-lg shadow max-h-64 object-cover w-full">
    </div>
</div>
@endsection
